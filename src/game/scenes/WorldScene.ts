  ...
  createWorldGround() {
    ...
    for (let ty = 0; ty < size; ty++) {
      for (let tx = 0; tx < size; tx++) {
        ...
        // 根据噪声值决定地形类型（大幅减少水域）
        const adjustedElevation = elevation * 0.6 + 0.3;
        
        if (adjustedElevation < -0.5) {
          // 深海（极少）
        ...
        } else if (adjustedElevation > 0.55) {
          // 山脉/岩石
        ...
        } else if (adjustedElevation > 0.42) {
          // 山地草地
        ...
        // 添加小花装饰（只在草地上）
        if (adjustedElevation >= -0.3 && adjustedElevation <= 0.42 && moisture > -0.4) {
